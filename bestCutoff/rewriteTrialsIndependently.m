...
function [contactsAuto] = rewriteTrialsIndependently(contactArray, allCutoffs) 
% Written by Garrett Flynn (5/14/19) 
 
% REWRITETRIALSINDEPENDENTLY converts an existing autocurated contact array into a 
% new contact array based on its prediction values and desired cutoffs for 
...
for i = 1:numTrials % We want to iterate by trial 
    ...
    elseif isfield(contactArray{i},'touchConfidence') 
        ...
        for j = 1:numel(contactArray{i}.touchConfidence) %Iterate through each point 
            ...
            if con(j) < allCutoffs(i)  
                % Non-Touch point 
                contactCell(j) = 0; 
            elseif con(j)  > allCutoffs(i) 
                % Touch point 
                contactCell(j) = 1; 
            else 
