...
function [metrics] = contact_metrics_analyzer_var(autoCon, manCon, T)
  ...
  for i = 1:numTrials
    ...
    elseif ~isempty(autoPoints) && ~isempty(manPoints)
        ...
        extraTouches(i) = length(autoOnset) - length(manOnset);
        disagreedWholeTouches(i) = abs(extraTouches(i));
        humanTouches(i) = length(manOnset);
    else
        agreeNum(i) = 0;
    end
  end
  ...
  metrics.FalseTouchPercent = 100*nansum(falseTouch)/(4000*length(manCon));
  metrics.MissedTouchPercent = 100*nansum(falseNonTouch)/(4000*length(manCon));
  ...
  extraTouches = nansum(extraTouches);
  metrics.extraTouches = extraTouches;
  wholeTouchDisagreement = nansum(disagreedWholeTouches)/(nansum(humanTouches));
  metrics.wholeTouchDisagreement = wholeTouchDisagreement;